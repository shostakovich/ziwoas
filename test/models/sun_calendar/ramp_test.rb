...
class SunCalendar::RampTest < ActiveSupport::TestCase
  cover "SunCalendar::Ramp*"

  test "ends of the ramp are the outer stops" do
    ramp = SunCalendar::Ramp.fetch(:amber)
  ...
  test "mixes between the two stops a fraction falls into" do
    ramp = SunCalendar::Ramp.new(%w[#000000 #ffffff])
  ...
  test "picks the right pair out of a longer ramp" do
    ramp = SunCalendar::Ramp.new(%w[#000000 #ff0000 #ffffff])
  ...
  test "offers the ramp as a css gradient for the legend" do
    assert_equal "linear-gradient(90deg, #000000, #ffffff)", SunCalendar::Ramp.new(%w[#000000 #ffffff]).css_gradient
  end

  test "knows the three named ramps and nothing else" do
    assert_equal %i[amber blue grey], SunCalendar::Ramp::STOPS.keys
    assert_raises(KeyError) { SunCalendar::Ramp.fetch(:violet) }
  end
end
