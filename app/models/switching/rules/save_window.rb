module Switching
  module Rules
    ...
    class SaveWindow
      ...
      def call
        on_minute  = Switching::Rule.minutes_from(@attrs[:on_at_time])
        off_minute = Switching::Rule.minutes_from(@attrs[:off_at_time])
        days       = @attrs[:days]
        group_id   = @group_id || SecureRandom.uuid

        Switching::Rule.transaction do
      ...
      def write(group_id, action, at_minute, days)
        rule = Switching::Rule.find_or_initialize_by(group_id: group_id, action: action)
