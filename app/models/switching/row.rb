...
module Switching
  class Row
    ...
    def self.build_all(plugs, now: Time.current)
      ...
      rules_by_plug  = Switching::Rule.where(plug_id: plug_ids)
                                 .order(:at_minute, :id).group_by(&:plug_id)
      states_by_plug = Plugs::State.where(plug_id: plug_ids).index_by(&:plug_id)
      commands_by_plug = Switching::Command.latest_per_plug(plug_ids)
      ...
      plugs.map do |plug|
        ...
        new(
          plug:         plug,
          entries:      Switching::Rules::Schedule.fold(rules),
          state:        states_by_plug[plug.id],
          last_command: commands_by_plug[plug.id],
          next_edge:    Switching::EdgeCalculator.new(rules: rules.select(&:enabled))
